...
export default function Referral() {
  const referralStats = [
    { level: 'Level 1', count: 15, commission: '5%', gradient: 'from-blue-500 to-blue-600', textColor: 'text-blue-100' },
    { level: 'Level 2', count: 22, commission: '3%', gradient: 'from-purple-500 to-purple-600', textColor: 'text-purple-100' },
    { level: 'Level 3', count: 10, commission: '1%', gradient: 'from-green-500 to-green-600', textColor: 'text-green-100' }
  ];
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        {referralStats.map((stat, index) => (
          <div key={index} className={`card bg-gradient-to-r ${stat.gradient} text-white shadow-lg`}>
              ...
              <tbody>
                {referralList.map((referral, index) => (
                  <tr key={index}>
                    ...
                    <td><span className={`badge ${referral.levelColor}`}>{referral.level}</span></td>
                    <td>{referral.joinDate}</td>
                    <td className="text-green-600 font-bold">{referral.totalCommission}</td>
                    <td><span className={`badge ${referral.statusColor}`}>{referral.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
